    ...
    // --- ROL Y USUARIO (CORREGIDO) ---
    // 1. Obtenemos el valor crudo del HTML (puede venir como 'Feligrés', 'Secretaria', etc.)
    const rawRol = document.body.dataset.rol || '';

    // 2. Normalizamos: Todo a minúsculas y quitamos tildes
    // Ej: "Feligrés" -> "feligres", "Secretaría" -> "secretaria"
    ...
    // Debug en consola para verificar qué está leyendo
    ...
    async function cargarParroquias() {
        try {
            ...
            datos.forEach(p => {
                ...
                // Usamos la variable normalizada 'rolUsuario'
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Validación de rol (Usando la variable normalizada)
        if (rolUsuario !== 'feligres' && rolUsuario !== 'secretaria') {
            console.warn(`Bloqueo de seguridad: Rol '${rolUsuario}' no autorizado.`);
            alert("Usted no puede hacer reservas.");
            
            // Intentar mostrar mensaje en la UI si existe el contenedor
        ...
        if (rolUsuario === 'secretaria' && idParroquiaUsuario) {
            // Esperamos un poco a que carguen los marcadores
            setTimeout(() => {
