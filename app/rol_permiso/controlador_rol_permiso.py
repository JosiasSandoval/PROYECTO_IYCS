from app.bd_sistema import obtener_conexion

# =========================
# CRUD - ROL
# =========================
def get_listar_rol():
    conexion = obtener_conexion()
    try:
        resultados = []
        with conexion.cursor() as cursor:
            cursor.execute("SELECT idRol, nombRol, estadoRol FROM rol")
            filas = cursor.fetchall()
            for fila in filas:
                resultados.append({
                    'id': fila[0],
                    'nombre': fila[1],
                    'estado': fila[2]
                })
        return resultados
    except Exception as e:
        print(f"Error al listar los roles: {e}")
        return []
    finally:
        if conexion:
            conexion.close()


def agregar_rol(nombRol):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("INSERT INTO rol (nombRol, estadoRol) VALUES (%s, TRUE)", (nombRol,))
        conexion.commit()
        return {"ok": True, "mensaje": "Rol agregado correctamente"}
    except Exception as e:
        print(f"Error al agregar rol: {e}")
        return {"ok": False, "mensaje": str(e)}
    finally:
        if conexion:
            conexion.close()


def actualizar_rol(idRol, nombRol):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("UPDATE rol SET nombRol = %s WHERE idRol = %s", (nombRol, idRol))
        conexion.commit()
        return {"ok": True, "mensaje": "Rol actualizado correctamente"}
    except Exception as e:
        print(f"Error al editar rol: {e}")
        return {"ok": False, "mensaje": str(e)}
    finally:
        if conexion:
            conexion.close()


def cambiar_estado_rol(idRol):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("SELECT estadoRol FROM rol WHERE idRol = %s", (idRol,))
            estado_actual = cursor.fetchone()
            if estado_actual is None:
                return {"ok": False, "mensaje": f"No existe rol con id {idRol}"}
            nuevo_estado = not estado_actual[0]
            cursor.execute("UPDATE rol SET estadoRol = %s WHERE idRol = %s", (nuevo_estado, idRol))
        conexion.commit()
        return {"ok": True, "mensaje": "Estado del rol actualizado correctamente"}
    except Exception as e:
        print(f"Error al cambiar estado del rol: {e}")
        return {"ok": False, "mensaje": str(e)}
    finally:
        if conexion:
            conexion.close()


def verificar_relacion_rol(idRol):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("""
                SELECT (
                    (SELECT COUNT(*) FROM rol_permiso WHERE idRol = %s) +
                    (SELECT COUNT(*) FROM rol_usuario WHERE idRol = %s)
                ) AS total_relaciones
            """, (idRol, idRol))
            resultado = cursor.fetchone()
            return resultado and resultado[0] > 0
    except Exception as e:
        print(f"Error al verificar las relaciones del rol: {e}")
        return False
    finally:
        if conexion:
            conexion.close()


def eliminar_rol(idRol):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("DELETE FROM rol WHERE idRol = %s", (idRol,))
        conexion.commit()
        return {"ok": True, "mensaje": "Rol eliminado correctamente"}
    except Exception as e:
        print(f"Error al eliminar rol: {e}")
        return {"ok": False, "mensaje": str(e)}
    finally:
        if conexion:
            conexion.close()


def busqueda_rol(busqueda):
    conexion = obtener_conexion()
    try:
        resultados = []
        with conexion.cursor() as cursor:
            cursor.execute("SELECT idRol, nombRol, estadoRol FROM rol WHERE nombRol LIKE %s", (f"%{busqueda}%",))
            filas = cursor.fetchall()
            for fila in filas:
                resultados.append({
                    'id': fila[0],
                    'nombre': fila[1],
                    'estado': fila[2]
                })
        return resultados
    except Exception as e:
        print(f"Error en la búsqueda de roles: {e}")
        return []
    finally:
        if conexion:
            conexion.close()


# =========================
# CRUD - PERMISO
# =========================
#No se usara
def get_listar_permiso():
    conexion = obtener_conexion()
    try:
        resultados = []
        with conexion.cursor() as cursor:
            cursor.execute("""
                SELECT idPermiso, tipoOperacion, nombAccion, nombTabla, descripcionPermiso, estadoPermiso 
                FROM permiso
            """)
            filas = cursor.fetchall()
            for fila in filas:
                resultados.append({
                    'id': fila[0],
                    'operacion': fila[1],
                    'accion': fila[2],
                    'tabla': fila[3],
                    'descripcion': fila[4],
                    'estado': fila[5]
                })
        return resultados
    except Exception as e:
        print(f"Error al listar los permisos: {e}")
        return []
    finally:
        if conexion:
            conexion.close()


def cambiar_estado_permiso(idPermiso):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("SELECT estadoPermiso FROM permiso WHERE idPermiso = %s", (idPermiso,))
            estado_actual = cursor.fetchone()
            if estado_actual is None:
                return {"ok": False, "mensaje": f"No existe permiso con id {idPermiso}"}
            nuevo_estado = not estado_actual[0]
            cursor.execute("UPDATE permiso SET estadoPermiso = %s WHERE idPermiso = %s", (nuevo_estado, idPermiso))
        conexion.commit()
        return {"ok": True, "mensaje": "Estado del permiso actualizado correctamente"}
    except Exception as e:
        print(f"Error al cambiar estado del permiso: {e}")
        return {"ok": False, "mensaje": str(e)}
    finally:
        if conexion:
            conexion.close()


# =========================
# RELACIÓN ROL - PERMISO
# =========================
def agregar_rol_permiso(idRol, idPermiso):
    conexion = obtener_conexion()
    try:
        with conexion.cursor() as cursor:
            cursor.execute("INSERT INTO rol_permiso (idRol, idPermiso) VALUES (%s, %s)", (idRol, idPermiso))
        conexion.commit()
        return {"ok": True, "mensaje": "Permiso asignado al rol correctamente"}
    except Exception as e:
        print(f"Error al agregar rol_permiso: {e}")
        return {"ok": False, "mensaje": str(e)}
    finally:
        if conexion:
            conexion.close()

# =========================
# MOSTRAR PERMISOS - ROL
# =========================
def get_listar_permisos():
    conexion = obtener_conexion()
    try:
        resultados = []
        with conexion.cursor() as cursor:
            cursor.execute("""
                SELECT idPermiso,nombPermiso, estadoPermiso 
                FROM permiso
            """)
            resultados=cursor.fetchall()
            for fila in resultados:
                resultados.append({
                    'id': fila[0],
                    'nombre': fila[1],
                    'estado': fila[2]
                })
        return resultados
    except Exception as e:
        print(f"Error al listar los permisos: {e}")
        return []
    finally:
        if conexion:
            conexion.close()

def mostrar_permisos_rol(idRol):
    conexion = obtener_conexion()
    try:
        permisos = []
        with conexion.cursor() as cursor:
            cursor.execute("""
                SELECT pe.idPermiso,r.nombRol, pe.nombAccion,pe.tipoOperacion, pe.nombTabla,pe.estadoPermiso
                FROM rol_permiso rp
                INNER JOIN rol r ON rp.idRol = r.idRol
                INNER JOIN permiso pe ON rp.idPermiso = pe.idPermiso
                WHERE r.idRol = %s
            """, (idRol,))  # <-- pasar el parámetro como tupla
            resultados = cursor.fetchall()
            
            for fila in resultados:
                permisos.append({
                    'id': fila[0],
                    'rol': fila[1],
                    'permiso': fila[2],
                    'operacion':fila[3],
                    'tabla':fila[4],
                    'estado':fila[5]
                })
        return permisos
    except Exception as e:
        print(f"Error al mostrar permisos del rol: {e}")
        return []
    finally:
        if conexion:
            conexion.close()

